	...
	if ( class_exists( 'WP_Customize_Section' ) && ! class_exists( 'Kirki_Installer_Control' ) ) {
		...
		class Kirki_Installer_Control extends WP_Customize_Control {
			...
			public function render_content() { ?>
				...
				<?php else : ?>
					<hr>
					<p><?php printf( esc_html__( 'The plugin is installed but not activated. Please <a href="%s">activate it</a>.', 'setwood' ), admin_url( 'plugins.php' ) ); ?></p>
				<?php endif;
			}
		}
	...
	if ( ! function_exists( 'kirki_installer_register' ) ) {
		...
		function kirki_installer_register( $wp_customize ) {
			...
			$wp_customize->add_section( 'kirki_installer', array(
				'title'       => '',
				'description' => esc_attr__( 'If you want to take full advantage of the options this theme has to provide, please install the Kirki plugin.', 'setwood' ),
				'priority'    => -10,
